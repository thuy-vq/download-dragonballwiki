...
const BASE_URL = "https://nettruyen.work/truyen-tranh/luc-do-toi-da-chuyen-sinh-thanh-slime/chuong-";

const START_CHAP = 1; // Chap bắt đầu
const END_CHAP = 137; // Chap kết thúc
const OUTPUT_DIR = "./toi-da-chuyen-sinh-thanh-slime";

// Các đuôi mở rộng cần thử (Nettruyen thường dùng gạch nối cho chap lẻ: chuong-1-5)
const TRY_SUFFIXES = ["", "-1", "-2", "-5"];

const CONCURRENT_LIMIT = 10; // Số ảnh tải song song
const IMG_RETRY_LIMIT = 3; // Thử lại ảnh nếu lỗi
const CHAP_TIMEOUT_MS = 120000; // 60s timeout cho 1 chap
const MAX_CHAP_RETRIES = 3; // Thử lại chap nếu lỗi

// --- SELECTOR (Chuẩn Nettruyen) ---
// Ảnh nằm trong .reading-detail > .page-chapter > img
const CONTENT_WRAPPER = ".reading-detail.box_doc";
const IMG_SELECTOR = ".page-chapter img";
...
async function downloadImage(url, folderPath, index, refererUrl) {
  ...
  if (
    !url ||
    (url.includes("cdn.nettruyen") === false && !url.startsWith("http"))
  )
    return true;
...
async function processOneChapter(page, currentUrl, folderName) {
  ...
  if (imgUrls.length === 0)
    throw new Error("Không tìm thấy ảnh nào (Selector sai hoặc dính Captcha)!");
  ...
  for (let i = 0; i < imgUrls.length; i += CONCURRENT_LIMIT) {
    ...
    const tasks = chunk.map((url, k) =>
      downloadImage(url, fullFolderPath, i + k + 1, currentUrl),
    );
        ...
        try {
          await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error("TIMEOUT")), CHAP_TIMEOUT_MS),
            ),
          ]);
